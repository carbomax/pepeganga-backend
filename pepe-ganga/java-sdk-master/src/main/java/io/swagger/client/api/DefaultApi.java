...
/**/
    ...
    public UserTestResponse CreateTestUser(String site_id, String accessToken) throws ApiException{
    	
    	Object localVarPostBody = null;
    	
    	 // verify the required parameter 'userId' is set
        if (site_id == null) {
            throw new ApiException(400, "Missing the required parameter 'site_id' when calling CreateTestUser");
        }
        
     // create path and map variables      
        String localVarPath = "/users/test_user/{access_token}"
        		.replaceAll("\\{" + "access_token" + "\\}", apiClient.escapeString(accessToken.toString()));;
        ...
        localVarQueryParams.addAll(apiClient.parameterToPairs("", "site_id", site_id));     
        ...
        GenericType<UserTestResponse> localVarReturnType = new GenericType<UserTestResponse>() {
        };
        return apiClient.invokeAPI(localVarPath, "POST", localVarQueryParams, localVarPostBody, localVarHeaderParams, localVarFormParams, localVarAccept, localVarContentType, localVarAuthNames, localVarReturnType);
