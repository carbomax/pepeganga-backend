...
public class MarginServiceImpl implements MarginService {
    ...
    public Margin createMargin(Margin margin, Integer profileId) {
        try {
            ...
            if (profile.isPresent()) {
                ...
                } else throw new ResponseStatusException(HttpStatus.NOT_MODIFIED,
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    String.format("Type of margin %s it not permitted. Try [%d, %d]", margin.getType(), MarginType.SIMPLE.getCode(), MarginType.PERCENT.getCode()));
        }
    }
        ...
        profileDb.ifPresent(profile -> {
            ...
            if (marginDb.isPresent()) {
                try {
                    ...
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("Type of margin %s it not permittedTry [%d, %d]", margin.getType(), MarginType.SIMPLE.getCode(), MarginType.PERCENT.getCode()));
                }
            } else
                throw new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("Margin not found with id %d", idMargin));
        });
        if(profileDb.isEmpty()){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("Margin not created, profile id not founded with id: %d", idProfile));
        }
        Optional<Margin> first = profileFinal.get().getMargins().stream().filter(margin1 -> margin1.getId().equals(idMargin)).findFirst();
        if (first.isPresent()) {
            return first.get();
        } else
            throw new ResponseStatusException(HttpStatus.NOT_MODIFIED, String.format("Margin not updated with id %d", idMargin));
    ...
    public void deleteMargin( Short idMargin) {
        if (marginRepository.existsById(idMargin)) {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("Margin not deleted with id %s, it not exist", idMargin));
    }
}
